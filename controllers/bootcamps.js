...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
    ...
    if (bootcamp) {
        ...
        if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
            next(
                new ErrorResponse(`The user with ID ${req.user.id} has is not authorized to update this bootcamp`, 401)
            );
        }
...
exports.deleteBootcamp = asyncHandler(async (req, res, next) => {
    ...
    if (bootcamp) {
        ...
        if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
            next(
                new ErrorResponse(`The user with ID ${req.user.id} has is not authorized to delete this bootcamp`, 401)
            );
        }
        res.status(200).json({ success: true, data: bootcamp });
    } else {
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
    const bootcamp = await Bootcamp.findById(req.params.id);
    if (bootcamp) {
        // make sure user is bottcamp owner
        if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
            next(
                new ErrorResponse(`The user with ID ${req.user.id} has is not authorized to upload a photo`, 401)
            );
        }
